...
//! # Balances Pallet
//!
//! The Balances pallet provides functionality for handling accounts and balances_old.
//!
//! - [`Config`]
//! - [`Call`]
//! - [`Pallet`]
...
//!
//! - Getting and setting free balances_old.
//! - Retrieving total, reserved and unreserved balances_old.
...
//!   accounting
//! (i.e. a difference between total issuance and account balances_old). Functions that result in an
...
//! imbalances between total issuance in the system and account balances_old. Must be used when a
...
//! - `set_balance` - Set the balances_old of a given account. The origin of this call must be root.
//!
//! ## Usage
//!
...
const LOG_TARGET: &str = "runtime::balances_old";
...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		/// The means of storing the balances_old of an account.
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		///   - Transferring balances_old to accounts that did not exist before will cause
		...
		/// Set the balances_old of a given account.
		///
		...
		///
		/// NOTE: This function only attempts to transfer _transferable_ balances_old. This means that
	...
	/// ```
	///
	/// But this comes with tradeoffs, storing account balances_old in the system pallet stores
	/// `frame_system` data alongside the account data contrary to storing account balances_old in the
	/// `Balances` pallet, which uses a `StorageMap` to store balances_old data only.
	/// NOTE: This is only used in the case that this pallet is used to store balances_old.
	#[pallet::storage]
	pub type Account<T: Config<I>, I: 'static = ()> =
		StorageMap<_, Blake2_128Concat, T::AccountId, AccountData<T::Balance>, ValueQuery>;

	/// Any liquidity locks on some account balances_old.
	...
	/// Named reserves on some account balances_old.
	#[pallet::storage]
	...
	impl<T: Config<I>, I: 'static> GenesisBuild<T, I> for GenesisConfig<T, I> {
		fn build(&self) {
			...
			assert!(
				...
				"duplicate balances_old in genesis."
			);
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	/// Get both the free and reserved balances_old of an account.
	...
	fn update_locks(who: &T::AccountId, locks: &[BalanceLock<T::Balance>]) {
		...
		// No way this can fail since we do not alter the existential balances_old.
